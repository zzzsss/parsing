...
//******************************************
//get output from the mach
//--must make sure the input x,y are enough
void Mach::evaluate(REAL* x,REAL* y,int size,int id,int od)
{
	if(id != idim || od != odim)
		Error("Dimension not for this machine.");
	int remain = size;
	while(remain > 0){
		int n=0;
		if(remain >= bsize)
		...
		this->SetDataIn(x);
		this->Forw(n);
		memcpy(y, this->data_out, odim*sizeof(REAL)*n);
		y += n*odim;
		x += n*idim;
	}
}
